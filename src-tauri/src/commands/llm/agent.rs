...
use async_openai::{
    ...
    types::{
        ...
        ChatCompletionTool, ChatCompletionToolType, FinishReason,
    },
    Client,
};
...
impl Agent {
        ...
        loop {
            ...
            match finish_reason.as_deref() {
                Some("tool_calls") if !tool_calls.is_empty() => {
                    ...
                    // 注意: async-openai 的 tool_calls 需要转换
                    messages.push(assistant_msg.build()
                        .map_err(|e| format!("构建助手消息失败: {}", e))?
                        .into());
                    
                    // 执行所有工具调用
                    for tool_call in &tool_calls {
                        let tool_id = tool_call["id"].as_str()
                        ...
                        eprintln!("[Agent]    参数: {}", serde_json::to_string_pretty(&arguments).unwrap_or_else(|_| arguments_str.to_string()));
                        
                        // 发送工具执行开始事件
                        self.window.emit("llm-tool-start", serde_json::json!({
                        ...
                        // 执行工具（异步，可能耗时）
                        let tool_result = match ToolExecutor::execute(&self.window, function_name, arguments.clone()).await {
                            Ok(result) => {
                        ...
                        // 发送工具执行完成事件
                        self.window.emit("llm-tool-done", serde_json::json!({
                            "message_id": self.message_id,
                            "tool_id": tool_id,
                            "tool_name": function_name,
                            "result": tool_result
                ...
                Some(reason) if reason.to_lowercase() == "stop" => {
                    ...
                    self.window.emit("llm-stream-done", &self.message_id)
                        .map_err(|e| format!("Emit error: {}", e))?;
                    break;
                }
                None => {
    ...
    async fn call_llm_stream(
        ...
        let chat_tools: Vec<ChatCompletionTool> = tools.into_iter()
        ...
        if !tool_calls.is_empty() {
            self.window.emit("llm-stream-tool-calls", serde_json::json!({
                "message_id": self.message_id,
    ...
    async fn emit_chunk(&self, content: &str, full_content: &str) -> Result<(), String> {
        self.window.emit("llm-stream-chunk", serde_json::json!({
            "message_id": self.message_id,
            "content": content,

...
impl ToolExecutor {
    ...
    pub async fn execute(
        ...
        eprintln!("[ToolExecutor] 执行工具: {} with args: {}", tool_name, arguments);
    ...
    async fn exec_list_files(args: Value) -> Result<String, String> {
        let options: ListFilesOptions = serde_json::from_value(args)
    ...
    async fn exec_read_file(args: Value) -> Result<String, String> {
        ...
        let path = args.get("path")
            ...
            .ok_or("缺少 path 参数")?
            .to_string();
        let max_size = args.get("max_size").and_then(|v| v.as_u64()).map(|n| n as usize);
    ...
    async fn exec_fetch_webpage(args: Value) -> Result<String, String> {
        let options: network::FetchWebpageOptions = serde_json::from_value(args)
            .map_err(|e| format!("解析参数失败: {}", e))?;
    ...
    async fn exec_run_command(args: Value) -> Result<String, String> {
        let options: RunCommandOptions = serde_json::from_value(args.clone())
            .map_err(|e| format!("解析参数失败: {}", e))?;
    ...
    fn extract_download_path(command: &str) -> Option<String> {
        ...
        if let Some(outfile_pos) = command.find("-OutFile") {
            ...
            // 查找引号或空格后的路径
            let path_start = after_outfile.find(|c: char| c == '"' || c == '\'' || !c.is_whitespace())?;
            let path = &after_outfile[path_start..];
            let path_end = path.find(|c: char| c == '"' || c == '\'' || c == ' ' || c == '\n' || c == '\r')
                .unwrap_or(path.len());
            let file_path = path[..path_end].trim_matches(|c| c == '"' || c == '\'').trim();
    ...
    fn verify_download_file(file_path: &str) {
        match std::fs::metadata(file_path) {
            Ok(metadata) => {
                ...
                if size > 0 {
                    eprintln!("[ToolExecutor] ✅ 下载验证成功: {} (大小: {} 字节)", file_path, size);
                } else {
                    eprintln!("[ToolExecutor] ⚠️ 警告: 文件存在但大小为 0 字节: {}", file_path);
                }
            }
            Err(e) => {
                eprintln!("[ToolExecutor] ⚠️ 警告: 无法验证文件是否存在: {} (错误: {})", file_path, e);
            }
        }
    }
}
